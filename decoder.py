...
class PixelDecoder(nn.Module):
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32):
        ...
        for i in range(self.num_layers - 1):
            ...
            self.deconvs.append(
                nn.ConvTranspose2d(num_filters, num_filters, kernel, stride=1, output_padding=output_padding) #stride=2
            )
        self.deconvs.append(
            nn.ConvTranspose2d(
                num_filters, num_out_channels, kernel, stride=2, output_padding=1 #1
            )
        )
