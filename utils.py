...
class ReplayBuffer(object):
    ...
    def __init__(self, obs_shape, action_shape, capacity, batch_size, device, is_framestack=False):
        ...
        self.env_id = np.empty((capacity, 1), dtype=np.int32)

        self.idx = 0
        self.last_save = 0
        self.full = False

    def add(self, obs, action, curr_reward, reward, next_obs, done, env_id):
        if self.is_framestack:
        ...
        np.copyto(self.env_id[self.idx], env_id)
    ...
    def sample(self, k=False):
        ...
        env_ids = torch.as_tensor(self.env_id[idxs], device=self.device)
        if k:
            return obses, actions, rewards, next_obses, not_dones, torch.as_tensor(self.k_obses[idxs], device=self.device),env_ids
        return obses, actions, curr_rewards, rewards, next_obses, not_dones, env_ids

    def sample_for_classfy(self, k=False):
        idxs = np.random.randint(
            0, self.capacity if self.full else self.idx, size=self.batch_size2
        )
        if self.is_framestack:
            obses = torch.as_tensor(self.obses[idxs][:, :-3], device=self.device).float()
            next_obses = torch.as_tensor(
...
class FrameStack(gym.Wrapper):
    ...
    def _get_obs(self):
        ...
        if self._frames[0].shape[0] < self._k * 3:
            tmp = np.concatenate(list(self._frames), axis=0)
        else: tmp = self._frames[0]
        return tmp
