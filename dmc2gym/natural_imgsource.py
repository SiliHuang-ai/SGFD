...
import skvideo
skvideo.setFFmpegPath('C:/Users/admin\AppData\Local\Programs\Python\Python37\Lib\site-packages\skvideo\io')
...
class RandomImageSource(ImageSource):
    ...
    def build_arr(self):
        ...
        for i in range(self.total_frames):
            ...
            if self.grayscale: self.arr[i] = np.expand_dims(cv2.resize(im, (self.shape[1], self.shape[0])),axis=-1)
            else:              self.arr[i] = cv2.resize(im, (self.shape[1], self.shape[0])) ## THIS IS NOT A BUG! cv2 uses (width, height)
