...
import utils
from sac_ae_stable import  Actor, Critic, LOG_FREQ, Actor2, Envclassify
from transition_model import make_transition_model
from reweighting import set_parameters, weight_learner
from stable_config import parser
...
class StableAgent(object):
    ...
    def __init__(
        ...
        #TODO
        self.rew_length=encoder_feature_dim
        if self.sep_rew_dyn:
        ...
        self.Envclassify=Envclassify(hidden_dim, encoder_feature_dim, env_num).to(device)
        self.criterion_train = nn.CrossEntropyLoss().to(device)

        self.Envclassify_optimizer = torch.optim.SGD(self.Envclassify.parameters(), 0.01,#learning_rate=0.01
                                    momentum=0.9,
                                    weight_decay=1e-4)
        ...
        self.actor = Actor(
            ...
            num_layers, num_filters, encoder_stride, batch_size, device
        ).to(device)
        ...
        set_parameters(device, init_steps)
        self.stable_args = parser.parse_args()
    ...
    def update_actor_and_alpha(self, obs, env_idx, L, step, replay_buffer, update_classfy):


        _, pi, log_pi, log_std, encoder_obs = self.actor.obs_act(obs, detach_encoder=True)
        actor_Q1, actor_Q2 = self.critic(obs, pi, detach_encoder=True)
        actor_Q = torch.min(actor_Q1, actor_Q2)
        logits_softmax = self.Envclassify(encoder_obs.detach())
        acc = (logits_softmax.topk(1, dim=1, largest=True, sorted=False)[1] == env_idx).sum() / len(env_idx)
        if update_classfy and acc<0.9 and step>100000:
            for tmp in range(10):
                obs2, env_idx2 = replay_buffer.sample_for_classfy()
                _, _, _, _, encoder_obs2 = self.actor.obs_act(obs2, detach_encoder=True)
                logits_softmax = self.Envclassify(encoder_obs2.detach())
                env_loss = self.criterion_train(logits_softmax, env_idx2.view(-1).long())
                self.Envclassify_optimizer.zero_grad()
                env_loss.backward()
                self.Envclassify_optimizer.step()
            logits_softmax = self.Envclassify(encoder_obs.detach())
            acc = (logits_softmax.topk(1, dim=1, largest=True, sorted=False)[1] == env_idx).sum() / len(env_idx)

            # tmp += 1
            # print('update_classify_step: ', str(tmp), ' acc: ', str(acc))
        self.acc=acc
        if acc>0.0:
            pre_feature, pre_weight, pre_min_loss = self.actor.pre_feature, self.actor.pre_weight, self.actor.min_loss
            softmax_weight, weight, pre_feature, pre_weight, min_loss = weight_learner(encoder_obs, pre_feature,
                                                                                       pre_weight, pre_min_loss,
                                                                                       self.stable_args,
                                                                                       global_weight=True,
                                                                                       step=step,
                                                                                       task_dir=self.task_dir,
                                                                                       model=self.Envclassify)
            self.actor.pre_weight.data.copy_(pre_weight)
            self.actor.pre_feature.data.copy_(pre_feature)
            self.actor.min_loss.data.copy_(min_loss)
            actor_loss = (self.alpha.detach() * log_pi - actor_Q).view(1, -1).mm(softmax_weight).mean()
        else:
    ...
    def update_encoder(self, obs, action, reward, aug_obs, aug_next_obs, L, step):
        ...
        perm = torch.from_numpy(np.random.permutation(batch_size)).type(torch.int64)
        h2 = h[perm]
    ...
    def update(self, replay_buffer, L, step):
        obs, action, _, reward, next_obs, not_done, env_idx = replay_buffer.sample()
        ...
        if step % self.actor_update_freq == 0:
            update_classfy= step % (self.actor_update_freq * 10)==0
            self.update_actor_and_alpha(obs, env_idx, L, step, replay_buffer, update_classfy)
